...
for repo in $(ls /Users/andrey_akatov/IWE/); do
  if [ -d /Users/andrey_akatov/IWE/$repo/.git ]; then
    commits=$(git -C /Users/andrey_akatov/IWE/$repo log --since="today 00:00" --oneline --no-merges 2>/dev/null)
...
grep -nE "→ ждёт|ждёт|dep:|блокер|blocked:|остановлен|ждёт согласования" \
  /Users/andrey_akatov/.claude/projects/*/memory/MEMORY.md 2>/dev/null
```
...
> **Условный шаг:** скрипт опционален. Если `/Users/andrey_akatov/IWE/DS-strategy/scripts/check-index-health.py` отсутствует — пропустить.

```bash
SCRIPT="/Users/andrey_akatov/IWE/DS-strategy/scripts/check-index-health.py"
