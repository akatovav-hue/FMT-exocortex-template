...
Коммиты за вчера по всем `/Users/andrey_akatov/IWE/*/` репо. Сопоставить с DayPlan.
...
**Проверка обновлений:** `cd /Users/andrey_akatov/IWE/FMT-exocortex-template && bash update.sh --check 2>&1`. Если доступно обновление → добавить в «Требует внимания»: «Доступно обновление IWE → `/iwe-update`».

**Проверка Base-репо (FPF, SPF, ZP):**
```bash
for repo in FPF SPF ZP; do
  dir="/Users/andrey_akatov/IWE/$repo"
...
```
Если есть новые коммиты → добавить в «Требует внимания»: «[repo] обновлён upstream → `cd /Users/andrey_akatov/IWE/[repo] && git pull --rebase`». После pull FPF/SPF → reindex: `bash /Users/andrey_akatov/IWE/DS-MCP/knowledge-mcp/scripts/selective-reindex.sh FPF` (или SPF). Если DS-MCP отсутствует → пометить «требуется ручной reindex».
