...
1. **Skeleton-marker:** есть ли `<!-- IWE-INITIAL-NEEDED -->` в `/Users/andrey_akatov/IWE/DS-strategy/docs/Strategy.md`? Маркер ставится seed-шаблоном при первом setup и удаляется после initial-сессии. Его наличие = Strategy.md ещё не наполнена реальным содержимым.
2. **Явный intent пользователя:** в сообщении есть «первая», «начальная», «initial», «day-0», «c нуля»?
3. **Файлы:** существует ли `/Users/andrey_akatov/IWE/DS-strategy/docs/Strategy.md` и/или хотя бы один `/Users/andrey_akatov/IWE/DS-strategy/current/WeekPlan W*.md`?
...
| Файлы есть, маркера нет, intent не указан, есть WeekPlan со `status: draft` | **weekly** | `/Users/andrey_akatov/IWE/FMT-exocortex-template/roles/strategist/prompts/strategy-session-weekly.md` |
...
Запиши ответы в `/Users/andrey_akatov/IWE/DS-strategy/docs/Strategy.md` по структуре:
- Видение (1 год)
...
Запиши в `/Users/andrey_akatov/IWE/DS-strategy/docs/Dissatisfactions.md` списком: каждая неудовлетворённость = 1-2 строки.
...
Запиши в `/Users/andrey_akatov/IWE/DS-strategy/current/WeekPlan W{N}.md` (где N — номер ISO-недели).

### 2.4. Обновление MEMORY.md (2 мин)

В `~/.claude/projects/-Users-andrey_akatov-IWE/memory/MEMORY.md` добавь раздел «РП текущей недели» со списком из 2.3.

### 2.5. Закрытие initial-сессии

1. **Удали skeleton-marker** из `/Users/andrey_akatov/IWE/DS-strategy/docs/Strategy.md` — строка `<!-- IWE-INITIAL-NEEDED: ... -->`. Без удаления skill будет каждый раз уходить в initial.
...
Если режим = weekly: загрузи `/Users/andrey_akatov/IWE/FMT-exocortex-template/roles/strategist/prompts/strategy-session-weekly.md` и следуй ему.
