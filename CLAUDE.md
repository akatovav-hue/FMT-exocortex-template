...
- **Flow (с 19.04.2026, WP-49):** `scripts/template-sync/` реализован (SC `LOCAL.SC.003`). Авторский IWE = source-of-truth → `template-sync.sh` → FMT → GitHub → `update.sh` → пользователи.
  1. L1 файл УЖЕ в `sync-manifest.yaml` → правим в корне → `bash scripts/template-sync/template-sync.sh --apply` → commit FMT
...
- **Режимы:** `--dry-run` = показать diff (без записи), `--apply` = применить + бэкап + validate, `--verify` = только validate-template.sh, `--rollback` = восстановить FMT из последнего бэкапа, `--bootstrap <path>` = разовое FMT→корень (для устаревших/отсутствующих файлов).
...
- **Детали архитектуры:** ADR-003 Template-sync контракт (FMT/docs/adr/ADR-003-template-sync.md).
