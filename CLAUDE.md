...
Рабочая директория: `/Users/andrey_akatov/IWE/` (не из sub-директорий). `/Users/andrey_akatov/IWE/memory/` = симлинк на auto-memory.
...
- `/Users/andrey_akatov/IWE/` — рабочая директория
...
### MEMORY = derived view (Staging S-37, 2026-05-11)

> **Принцип.** MEMORY — lightweight cache of inferred state поверх registries и DS-данных, **не canonical source**. Источник истины фиксированных сущностей (Agents, Brands, Clients, Companies, IT-Systems, Suppliers/Factories, Decisions, Risks, Roles, Processes) — registries в `DS-totallook/org-model/registries/` + `DS-totallook/org-model/agents/REGISTRY.md` + `DS-strategy/docs/decisions-index.md` + `DS-strategy/docs/*-risk-register.md`.

> **Полная карта registries и MEMORY-derived dependencies:** `memory/memory-architecture.md` (warm-tier).

**Правила citation (Phase 1 — defensive, без новой инфраструктуры):**

1. При упоминании фиксированной сущности в MEMORY → **markdown-link на anchor в registry**:
   - Агент: `[Цыганова](DS-totallook/org-model/agents/REGISTRY.md#цыганова-ольга-валерьевна)`
   - Бренд: `[Adidas](DS-totallook/org-model/registries/brands-portfolio.md#adidas)`
   - Компания: `[Эланра](DS-totallook/org-model/registries/legal-entities.md#эланра)`
   - Фабрика: `[XYZ](DS-totallook/org-model/registries/suppliers-portfolio.md#xyz)`
   - Клиент: `[CCC](DS-totallook/org-model/registries/clients-master.md#ccc)`
   - Решение: `[DEC.9](DS-strategy/docs/decisions-index.md#dec-9)`

2. **Антипаттерн — fake citation (запрещено):** приписывать MEMORY атрибуцию, которой там нет. Если фактический источник — рассуждение по аналогии, требуется маркер `[моё рассуждение, не из источника]`.

3. **Forward consult rule (mechanical step):** при применении PD.WP-кандидата №32 «6-link chain» к Шагу 3 (маппинг текущих исполняемых ролей) — **сначала Read REGISTRY**, потом mapping. Не выполняется «по памяти».

4. **При создании нового memory-entry** с упоминанием фиксированной сущности — citation обязателен; entry без anchor проходит review при Week Close.

**Phase 2 (drift-detection extension) — план для FMT:**
- Расширить `sync-manifest.yaml` записями типа `source: registries/*.md, derived: memory/*.md, check: agent-mention-cross-ref`
- `iwe-audit.sh` flag-ит memory entries с stale citations или missing anchors
- Day Open / Week Close surface-ят flag-list для batch update

**Why:** 11.05 за одну сессию — 4 fake-citation incidents (Зорина→сертификация, Цыганова→импорт-менеджер, Цыганова scope только Люкскидс, Федькин→менеджер взрослого). Все из-за работы «по памяти» без REGISTRY consult. Структурный fix: citation discipline + Phase 2 drift detection. См. также `feedback_verify_name_in_primary_source.md` + `feedback_position_role_legal_distinction.md`.

**Promotion path:** этот принцип — staging candidate. Validation 2-4 недели → промоция в FMT через `template-sync.sh` (`memory-lifecycle-spec.md` extension).
